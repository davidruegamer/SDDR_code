...
train = pd.read_csv("data/airfoil/train.csv")
test = pd.read_csv("data/airfoil/test.csv")

x_train = train[["V1","V2","V3","V4","V5"]]
x_test = test[["V1","V2","V3","V4","V5"]]
y_train = train["V6"].to_numpy().reshape(-1,1)
y_test = test["V6"].to_numpy().reshape(-1,1)
...
for i in range(20):
    ...
    numpy.savetxt('test_airfoil.out', res, delimiter=',')
